...
이 세 스킬 조합은 이 문제를 "문서를 예쁘게 쓰는 것"이 아니라 "현재 truth를 명시적으로 남기는 것"으로 다룹니다.
...
flowchart LR
    ...
    H --> P["lg-ontology"]
    P --> Q["graph_projection/nodes.jsonl"]
    P --> R["graph_projection/edges.jsonl"]
    P --> S["graph-style neighborhood / path inspection"]
```

읽는 법은 단순합니다.

- `bootstrap`는 레포의 운영 기준선과 현재 상태 문서를 만듭니다.
- `core ontology`는 문서 내용을 구조화된 사실 레이어로 바꿉니다.
- `lg`는 그 위에 graph projection과 graph-style inspection을 얹습니다.
- 셋이 겹치는 것처럼 보여도, 실제로는 `운영 정렬`, `지식 구조화`, `그래프 탐색 확장`으로 책임이 나뉩니다.
...
flowchart TD
    ...
    B -->|"멀티홉 경로, neighborhood, graph inspection 필요"| E["lg-ontology 적용"]

    C --> F["AGENTS.md 갱신"]
    C --> G["CURRENT_STATE / ARCHITECTURE 갱신"]
    C --> H["legacy / archive 분류"]
    C --> I["minimal intelligence contracts 정리"]

    D --> J["entity / document registry 생성"]
    D --> K["claim / evidence 연결"]
    D --> L["segment 생성"]
    D --> M["accepted claim 기반 derived edge 생성"]

    E --> N["graph projection 생성"]
    E --> O["multi-hop path / neighborhood inspection"]

    F --> P["새 세션 에이전트가 빠르게 맥락 파악"]
    G --> P
    H --> P
    I --> P
    J --> Q["근거 추적 가능한 지식 레이어"]
    K --> Q
    L --> Q
    M --> Q
    N --> R["graph-assisted exploration"]
    O --> R
```

핵심은 `항상 셋 다 돌리는 구조`가 아니라, 변화의 성격에 따라 필요한 스킬을 다시 적용하는 구조입니다.
...
### bootstrap만 쓸 때, ontology까지 쓸 때, graph projection까지 쓸 때의 차이
...
flowchart LR
    ...
    L["bootstrap + ontology + graph projection"] --> M["graph_projection nodes / edges"]
    L --> N["multi-hop path inspection"]
    L --> O["neighborhood exploration"]
    L --> P["baseline-vs-graph comparison"]
```
...
- 멀티홉 경로와 graph-style inspection까지 필요하면 `lg`
- 전체 흐름이 필요하면 `bootstrap -> ontology -> lg`
...
- [repo-docs-intelligence-bootstrap](C:/python_Github/repo-docs-ontology-skills/repo-docs-intelligence-bootstrap)
- [lightweight-ontology-core](C:/python_Github/repo-docs-ontology-skills/lightweight-ontology-core)
- [lg-ontology](C:/python_Github/repo-docs-ontology-skills/lg-ontology)
...
### Role split across the three skills
...
- `core ontology` turns document content into a structured fact layer
- `lg` adds graph projection and graph-style inspection on top
- they may seem adjacent, but in practice they divide into `operational alignment`, `knowledge structuring`, and `graph exploration`
...
flowchart TD
    ...
    B -->|"Need multi-hop paths, neighborhoods, or graph inspection"| E["Apply lg-ontology"]

    C --> F["Refresh AGENTS.md"]
    C --> G["Refresh CURRENT_STATE / ARCHITECTURE"]
    C --> H["Classify legacy / archive"]
    C --> I["Refresh minimal intelligence contracts"]

    D --> J["Create entity / document registries"]
    D --> K["Link claims and evidence"]
    D --> L["Generate segments"]
    D --> M["Generate derived edges from accepted claims"]

    E --> N["Create graph projection"]
    E --> O["Run multi-hop path / neighborhood inspection"]

    F --> P["New agent sessions understand context faster"]
    G --> P
    H --> P
    I --> P
    J --> Q["Traceable knowledge layer"]
    K --> Q
    L --> Q
    M --> Q
    N --> R["Graph-assisted exploration"]
...
The key idea is not "always run all three." The key idea is to re-apply the right skill based on the kind of change the project has gone through.
...
### Bootstrap only vs bootstrap plus ontology vs graph projection
...
- if you also need `multi-hop paths and graph-style inspection`, add `lg`
- if you need the full layered flow, go `bootstrap -> ontology -> lg`
