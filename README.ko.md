...
_DocTology 워크벤치 질문 작업공간 — 현재 위키를 확인하고, repo-local lexical preview와 graph hint를 검토한 뒤, durable synthesis는 LLM-first query 또는 proposal workflow로 넘깁니다._

현재 workbench의 실제 성격은 자유 대화형 LLM 작업공간이라기보다, 생성된 위키와 관련 preview를 읽고 검토하는 화면에 가깝습니다. 즉, 아직 능동적인 conversational chat 면은 아니며, preview-only route가 semantic analysis를 LLM-grounded answer처럼 저장해서도 안 됩니다.
...
3. LLM-first compile/query workflow로 source-backed evidence에서 위키를 성장시킴
4. complexity가 커질 때만 route receipts, graph projection, operator workflow를 붙임
...
현재 안전 규칙은 이렇습니다. Semantic growth는 LLM-first이면서 proposal-aware여야 합니다. Deterministic script는 source 등록, citation anchor 생성, index refresh, contract validation까지는 할 수 있지만, lexical output이나 graph output을 semantic truth로 바꾸면 안 됩니다. Compile 결과는 active concept/entity/project page를 직접 고치기 전에 human-review proposal이 되어야 합니다.
...
- **운영/유지보수:** `ontology-pipeline-operator`
...
- `llm-wiki-ontology-ingest`로 source page와 citation anchor를 등록/투영
- LLM compile/query workflow로 먼저 wiki map을 읽고, 그다음 page body와 source citation을 읽기
- 제안된 semantic update를 검토/apply한 뒤 질문·분석 워크플로로 위키를 계속 키우기
...
`wikiconfig.json`에 설정된 helper LLM은 선택적 accelerator입니다. helper LLM이 꺼져 있거나 없으면 script는 명시적인 prompt/bundle material을 주변 chat agent에게 넘겨야 하고, chat agent가 repo contract, wiki, ontology, source page, citation anchor를 직접 읽어 LLM-first semantic work를 수행할 수 있습니다. `wikiconfig.json`은 local/private 파일이며, 커밋되는 템플릿은 `wikiconfig.example.json`입니다.
...
canonical repo-local skillset은 `.agents/skills/` 아래에 있습니다. `~/.codex/skills` 아래의 installed copy는 로컬 설치본일 뿐입니다.
...
3. **DuckDB를 분석 창고로 사용**
   - source registry mirror, page coverage snapshot, audit 중심 점검

Claim, entity, relation은 canonical ontology JSONL과 proposal-reviewed wiki update에 속합니다. Bootstrap DuckDB mirror는 derived analytics aid이지 semantic truth layer가 아닙니다.
...
- 생성된 위키를 확인하고
- repo-local lexical preview와 graph/source hint를 검토하고
- durable synthesis를 LLM-first query 또는 proposal workflow로 넘기는
