...
Create a fresh markdown-first LLM Wiki workspace that is ready for Codex-style maintenance. The skill scaffolds the folder structure, `AGENTS.md`, starter `README.md`, local CLI, template files, and meta pages so the next agent can operate the vault consistently. The default profile is now `llm-first-ontology`: a strict no-fallback LLM-first ontology scaffold with contract-only YAML, proposal lifecycle policy, proposal/review registries, ambient chat-agent semantic handoff, and optional helper-LLM semantic workflows.
...
3. Choose the profile:
   - `llm-first-ontology` as the default for strict no-fallback LLM Wiki repos with contract-only YAML, proposal lifecycle, source/citation substrate, ambient chat-agent handoff, and optional helper-LLM semantic workflows
   - `wiki-plus-ontology` only for legacy ontology-ready repos with `warehouse/jsonl/` and minimal intelligence manifests; it emits a deprecation warning
...
5. Inspect the generated tree and verify that these exist:
   ...
   - for `llm-first-ontology`, also verify:
     ...
     - `warehouse/jsonl/`
   - for legacy `wiki-plus-ontology`, also verify:
...
The default is now the strict LLM-first ontology scaffold. For an explicit strict scaffold:
...
For the legacy three-layer ontology-ready scaffold:
...
- default strict ontology-ready `warehouse/jsonl/`, `intelligence/`, LLM compile/query scripts, proposal review CLI, and validators
...
- optional legacy `wiki-plus-ontology` `state/` and lightweight SQLite/DuckDB helper files
...
2. Run the bootstrap script in a temporary directory for `llm-first-ontology`.
3. Run the generated validators in that temporary directory.
